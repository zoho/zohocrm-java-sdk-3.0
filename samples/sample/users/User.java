...
import com.zoho.crm.api.users.TerritoryActionResponse;

import com.zoho.crm.api.users.TerritoryActionWrapper;

import com.zoho.crm.api.users.TerritoryBodyWrapper;

import com.zoho.crm.api.users.TerritoryResponseWrapper;

import com.zoho.crm.api.users.TerritorySuccessResponse;

import com.zoho.crm.api.users.Theme;
import com.zoho.crm.api.users.TransferActionResponse;
import com.zoho.crm.api.users.TransferActionWrapper;
import com.zoho.crm.api.users.TransferBodyWrapper;
import com.zoho.crm.api.users.TransferSuccessResponse;
import com.zoho.crm.api.users.TransferringResponseWrapper;
import com.zoho.crm.api.users.UsersOperations;

import com.zoho.crm.api.users.UsersOperations.GetTerritoriesofUserParam;
...
public class User
{
	...
	public static void createUser() throws Exception
	{
		...
		role.setId(34770610026008l);

		user1.setRole(role);

		user1.setFirstName("TestUser");

		user1.setEmail("testuser112321@zoho.com");
...
//		user1.addKeyValue(apiName, value);
	...
	public static void getTerritoriesOfUser(Long userId) throws Exception
	{
		...
		// Call getTerritoriesOfUser method that takes ParameterMap instance and HeaderMap instance as parameters
		APIResponse<ResponseHandler> response = usersOperations.getTerritoriesOfUser(userId, paramInstance);
		...
		if (response != null)
		{
			...
			if (response.isExpected())
			{
				...
				if (responseHandler instanceof TerritoryResponseWrapper)
				{
					// Get the received ResponseWrapper instance
					TerritoryResponseWrapper responseWrapper = (TerritoryResponseWrapper) responseHandler;

					// Get the list of obtained Territory instances
					List<com.zoho.crm.api.users.Territory> usersTerritory = responseWrapper.getTerritories();

					for (com.zoho.crm.api.users.Territory territory : usersTerritory)
					{
						...
						com.zoho.crm.api.users.Territory reportingTo = territory.getReportingTo();
	...
	public static void updateTerritoriesOfUser(Long userId) throws Exception
	{
		// Get instance of UsersOperations Class
		UsersOperations usersOperations = new UsersOperations();

		// Get instance of TerritoryBodyWrapper Class that will contain the request body
		TerritoryBodyWrapper request = new TerritoryBodyWrapper();

		// List of User instances
		List<com.zoho.crm.api.users.Territory> userTerritoryList = new ArrayList<com.zoho.crm.api.users.Territory>();

		// Get instance of Territory Class
		com.zoho.crm.api.users.Territory territory = new com.zoho.crm.api.users.Territory();
		...
		// Call updateTerritoriesOfUser method that takes userId and TerritoryBodyWrapper instance as parameter
		APIResponse<ActionHandler> response = usersOperations.updateTerritoriesOfUser(userId, request);
		...
		if (response != null)
		{
			...
			if (response.isExpected())
			{
				...
				if (actionHandler instanceof TerritoryActionWrapper)
				{
					// Get the received TerritoryActionWrapper instance
					TerritoryActionWrapper responseWrapper = (TerritoryActionWrapper) actionHandler;

					// Get the list of obtained TerritoryActionResponse instances
					List<TerritoryActionResponse> actionResponses = responseWrapper.getTerritories();

					for (TerritoryActionResponse actionResponse : actionResponses)
					{
						// Check if the request is successful
						if (actionResponse instanceof TerritorySuccessResponse)
						{
							// Get the received SuccessResponse instance
							TerritorySuccessResponse successResponse = (TerritorySuccessResponse) actionResponse;
	...
	public static void getSpecificTerritoryOfUser(Long userId, Long territoryId) throws Exception
	{
		...
		APIResponse<ResponseHandler> response = usersOperations.getSpecificTerritoryOfUser(territoryId, userId);
	...
	public static void getValidateTerritoriesBeforeTransferring(Long userId) throws Exception
	{
		// Get instance of UsersOperations Class
		UsersOperations usersOperations = new UsersOperations();

		// Call getValidateTerritoriesBeforeTransferring method that takes userId as parameters
		APIResponse<ResponseHandler> response = usersOperations.getValidateTerritoriesBeforeTransferring(userId);
		...
		if (response != null)
		{
			...
			if (response.isExpected())
			{
				...
				if (responseHandler instanceof TransferringResponseWrapper)
				{
					// Get the received TransferringResponseWrapper instance
					TransferringResponseWrapper responseWrapper = (TransferringResponseWrapper) responseHandler;

					// Get the list of obtained Territory instances
					List<com.zoho.crm.api.users.Territory> usersTerritory = responseWrapper.getValidateBeforeTransfer();
					...
					for (com.zoho.crm.api.users.Territory territory : usersTerritory)
					{
						...
						System.out.println("User Territory Records: " + territory.getRecords());
					}
				}
				// Check if the request returned an exception
				else if (responseHandler instanceof APIException)
	...
	public static void getValidateSpecificTerritoryBeforeTransferring(Long userId, Long territoryId) throws Exception
	{
		// Get instance of UsersOperations Class
		UsersOperations usersOperations = new UsersOperations();

		// Call getValidateTerritoriesBeforeTransferring method that takes territoryId and userId as parameters
		APIResponse<ResponseHandler> response = usersOperations.getValidateSpecificTerritoryBeforeTransferring(territoryId, userId);
	...
	public static void delinkAndTransferTerritoriesOfUser(Long userId) throws Exception
	{
		// Get instance of UsersOperations Class
		UsersOperations usersOperations = new UsersOperations();
		...
		com.zoho.crm.api.users.User transferToUser = new com.zoho.crm.api.users.User();
		...
		// Call delinkAndTransferTerritoriesOfUser method that takes userId and TransferBodyWrapper instance as parameter
		APIResponse<ActionHandler> response = usersOperations.delinkAndTransferTerritoriesOfUser(userId, request);
		...
		if (response != null)
		{
			...
			if (response.isExpected())
			{
				...
				if (actionHandler instanceof TransferActionWrapper)
				{
					...
					{
						// Check if the request is successful
						if (actionResponse instanceof TransferSuccessResponse)
						{
							// Get the received SuccessResponse instance
							TransferSuccessResponse successResponse = (TransferSuccessResponse) actionResponse;
	...
	public static void delinkAndTransferSpecificTerritoryOfUser(Long userId, Long territoryId) throws Exception
	{
		// Get instance of UsersOperations Class
		UsersOperations usersOperations = new UsersOperations();
		...
		// Call delinkAndTransferSpecificTerritoryOfUser method that takes territoryId, userId and TransferBodyWrapper instance as parameter
		APIResponse<ActionHandler> response = usersOperations.delinkAndTransferSpecificTerritoryOfUser(territoryId, userId, request);
